...
def show_country(df_country):
    ...
    fig = px.treemap(
        ...
        title=str(round(df_country["Trade Value"].sum() / 1e9, 2))
        + "B"
        + " EXPORTS OEC"
        if df_country["Trade Value"].sum() > 1e9
        else str(round(df_country["Trade Value"].sum() / 1e6, 2))
        + "M"
        + " EXPORTS OEC",
        labels={
            "Section": "Categoría",
...
def show_country_palo(df_country, name_df, random_colors):
    ...
    fig = px.treemap(
        ...
        title=str(round(df_country["value"].sum() / 1e9, 2)) + f"B {name_df}"
        if df_country["value"].sum() > 1e9
        else str(round(df_country["value"].sum() / 1e6, 2)) + f"M {name_df}"
        if df_country["value"].sum() > 1e6
        else str(round(df_country["value"].sum() / 1e3, 2)) + f"mil {name_df}"
        if df_country["value"].sum() > 1e3
        else str(round(df_country["value"].sum(), 2)) + f" {name_df}",
